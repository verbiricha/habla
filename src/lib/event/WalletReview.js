...
import { getEventId, getMetadata, encodeNaddr, findTag } from "../../nostr";
...
export function isWalletReview(props) {
  return findTag(props.event.tags, 'verdict') !== undefined;
}
...
export function WalletReview({
  ...
  metadata.verdict = findTag(event.tags, 'verdict');
      ...
      >
        ...
        <Link to={href}>
          <Heading fontFamily="var(--article-heading)" as="h1">
            {metadata.title} {`(verdict: ${metadata.verdict})`}
          </Heading>
          {!isPreview && seenIn}
